...
#
# Everything below works on `reports.AssetRecord`. A `Destination` enum and a
# parallel farm used to live above it, answering the same question a second way
# with no caller; it went with `routing.py`.
...
def _launcher_script() -> str:
    ...
    # The checkout's `cli.py` shim when this is a checkout, the installed
    # package's own module otherwise. Both are runnable on their own; the file
    # that used to be one directory up from here is now one or the other.
    checkout = Path(__file__).resolve().parents[2] / "cli.py"
    entry = shlex.quote(str(checkout if checkout.is_file() else Path(__file__).resolve().parent / "cli.py"))
    return f"""#!/bin/sh
